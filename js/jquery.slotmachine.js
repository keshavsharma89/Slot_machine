...
(function init($, window, document, undefined) {
    ...
    var SlotMachine = (function () {
        ...
        _createClass(SlotMachine, [{
            ...
            key: 'setRandomize',
            value: function setRandomize(rnd,line) {
			...
			key: 'unsetRandom',
			value: function unsetRandom(){
				this.settings.randomize=null;
				this.settings.online=0;
            ...
            value: function shuffle(spins, onComplete) {
                ...
                if(this.settings.online == 0){
					// addeed this to get even or odd number so that sometime we can stop the machine in ceter and somtime in front of the top line or bottom line
					let tmpnum=Math.floor(Math.random() * 1000);
					if((tmpnum%2)==0){
						this.settings.online =2;
						this.$container.css('position', 'relative');
						this.$container.css('top', '-50px');
					}else{
						let tmpnum2=Math.floor(Math.random() * 1000);
						if((tmpnum2%2)==0){
							this.settings.online =1;
						}else{
							this.settings.online =3;
						}
						this.$container.css('position', 'relative');
					...
					if(this.settings.online==1){
						this.$container.css('position', 'relative');
						this.$container.css('top', '0px');
					}else if(this.settings.online==2){
						+4;
						this.$container.css('position', 'relative');
						this.$container.css('top', '-50px');
					}else if(this.settings.online==3){
						+4;
						this.$container.css('position', 'relative');
						this.$container.css('top', '0px');
					}
